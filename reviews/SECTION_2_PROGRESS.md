...
| TF-NT-2 | [s] | **Re-assessed (deeper) → kept deferred, cosmetic/low-ROI.** Newtypes (`ControlScore`/`DisplayImportance`/`ProjectedPower`, `#[serde(transparent)]`, no arithmetic impls) exist in `src/analysis/scores.rs`. But the "wrap the struct fields" framing under-counts the cascade: `total_projection` is a **method** (`sector_model/mod.rs:1277`), `display_importance` a **free fn** (`importance.rs:132`) — not stored fields — so wrapping their returns ripples to ~15+ arithmetic/comparison sites (`importance.rs:134` `* sqrt`, `prose.rs:205` `partial_cmp`, `power_projection.rs:53/167`, `analytics.rs:255`, `render.rs:153`, `diff.rs:730/734`, `world_ecs.rs:180`), each needing `.get()`. Three separate stored `total_projection: f32` fields live in **serialized** structs (`analytics.rs:106`, `world_ecs.rs:99`, `diff.rs:231`). | Apply the newtypes only if/when a consumer actually compares scores across the three types (none does today). Until then it is pure type-decoration over the serialized analysis layer — wide cascade, golden-adjacent, zero behaviour change. | The infra (`scores.rs`) stays; site adoption is the deferred part. |
