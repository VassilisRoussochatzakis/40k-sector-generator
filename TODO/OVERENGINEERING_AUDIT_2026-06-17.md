...
- [ ] `yagni` ✓ **−27** `viewer/src/app/layout.rs` — `TopBar`/`MainView` single-field `&mut App` delegators, one call site each. Make free fns `pub(crate)`, call directly at app/mod.rs:191-192.
...
- [ ] `yagni` ✓ **−6** `viewer/src/app/types.rs:14` — `ExportJobResult` 3 variants nothing discriminates → `struct ExportJobResult(String)`. Keep builder's (it IS discriminated).
- [ ] `yagni` ✓ **−5** `src/export/system_map.rs:44` — `RESOLUTION_720P/1440P/4K` test-only; inline 1/2/3. Keep `MAX_SCALE`.
- [ ] `delete` · **−5** `src/validate/validation.rs:712` — `ValidationCode` `Display` impl (always built via `.as_slug().to_string()`).
- [ ] `yagni` · **−5** `builder/src/builder/panels/map/dialogs.rs:243` — `CollisionAction` single-variant enum = glorified bool.
- [ ] `yagni` · **−5** `src/model/sector_model/routes_view.rs:143` — `RouteType::pattern_key` one-line wrapper over `key()`, 1 caller.
- [ ] `delete` · **−4** `builder/src/builder/workspace.rs:72` — `BuilderWorkspace::get(idx)`, no caller; doc references a method that doesn't exist.
- [ ] `delete` · **−4** `builder/src/builder/derivation_cache.rs:355` — redundant match arm (dead `!stale.contains` guard; next arm identical).
- [ ] `yagni` · **−4** `src/model/sector_model/routes_view.rs:30` — `GeneratedRoute::pattern` no-salt shim, 0 callers.
- [ ] `delete` · **−3** `builder/src/builder/panels/interestingness.rs:589` — `_force_report_use` no-op that only silences an unused import; drop both.

## Duplication / shrink (quality, scanner-confident)

- [ ] `shrink` · **−33** `builder/src/builder/panels/relations.rs:653` — `attitude_combo`/`treaty_combo` identical bar enum → one generic combo.
- [ ] `shrink` · **−30** `builder/src/builder/panels/economy.rs:921` — `show_tech_rows`/`show_pop_rows` byte-identical bar map field → one helper.
- [ ] `shrink` · **−25** `tests/it/golden_png.rs:87` + `svg_export_tests.rs` — inlined blake3-pin block → use existing `assert_blake3_golden`, promote to shared.rs.
- [ ] `shrink` · **−15** `tests/it/relations_tests.rs:25` (+ personae/hooks/economy) — `gen_sector` copied verbatim in 4 files → shared.rs.
- [ ] `shrink` · **−12** `tests/it/loading_tests.rs:12` + `cli_behavior.rs` — `copy_dir_all` defined twice → shared.rs.
- [ ] `shrink` · **−12** `tests/it/route_monotonicity.rs:25` (+3) — `fixture_dir`/`fixture_project` re-defined → import shared.
- [ ] `shrink` · **−10** `tests/it/random_sector_tests.rs:324` — 3rd hand-rolled recursive `walkdir`; flat `read_dir` if presets aren't nested.
- [ ] `shrink` · **−8** `tests/it/analytics_and_presets.rs:7` — `presets_dir`/`goldens_dir` each defined twice → shared.rs.
- [ ] `shrink` · **−8** `src/model/sector_model/mod.rs:660` — `RouteType::key()` duplicates `as_slug()` slugs (low: parity-test-guarded, many callers).
- [ ] `shrink` · **−6** `builder/src/builder/state/selection.rs:66` — nav-stack cap idiom triplicated (`remove(0)` is O(n)).
- [ ] `shrink` · **−6** `builder/src/builder/project_io.rs:273` — `toml_err(&str)` subsumes `parse_err(&'static str)`.

## Stdlib / native micro-swaps

- [ ] `native` · **−6** `builder/src/builder/panels/map/theme.rs:755` — `parse_hex` → `Color32::from_hex` (low: clippy paint-ban may block).
- [ ] `shrink` · **−3** `builder/src/builder/project_io.rs:811` — `blake3_digest` wrapper (adds `blake3:` prefix; 1 caller).
- [ ] `shrink` · **−3** `viewer/src/factions_overview.rs:932` — `join_static_list` → inline `slice::join`.
- [ ] `shrink` · **−2** `src/analysis/personae.rs:382` — `node_presence<S,W>` generics are noise → take 2 `usize`.
- [ ] `stdlib` · **−2** `builder/src/builder/panels/map/cache.rs:18` — `.map(..).unwrap_or(true)` → `Option::is_none_or` (already the idiom here).
- [ ] `stdlib` · **−1** `builder/src/builder/panels/invariants.rs:313` — `splitn(2,'.')`+`.next()?`×2 → `str::split_once`.
- [ ] `stdlib` · **−0** `src/gen/world_pool.rs:376` — `partial_cmp().unwrap_or(Equal)` → `f64::total_cmp` (drops an import; deterministic NaN; net 0 lines).
