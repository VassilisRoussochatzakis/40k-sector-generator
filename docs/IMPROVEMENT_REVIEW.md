...
2. **`B-S3` / `A4` / `C6` / `E6` enum→slug boilerplate ×20+** — every status enum hand-writes `as_slug()` + `Display` + a parallel parse table (drift hazard: a new variant silently fails to parse). Codebase already accepts macros (`score_newtype!`). → add `enum_slug! { … }` (or `strum::EnumString`/`AsRefStr`). Deletes hundreds of lines.
