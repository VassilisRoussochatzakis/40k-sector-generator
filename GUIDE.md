...
* `handle_click_select_focuses_system`, `handle_click_shift_adds_to_selection`, `handle_drag_drop_move_succeeds`, `handle_drag_drop_collision_arms_dialog`, `handle_drag_drop_out_of_bounds_rejected`, `apply_rect_select_picks_systems_in_box`, `map_cache_refresh_populates_subsectors`, `map_cache_stable_across_idempotent_calls` in [builder/src/builder/panels/map.rs](builder/src/builder/panels/map.rs).
...
| C7 power-projection overlay | Three-way overlay toggle (OFF / POWER PROJECTION / INFLUENCE FIELD) backed by `BuilderState::control_overlay: ControlOverlay`. POWER PROJECTION runs `sectorforge::power_projection::project_sector`, normalises per-system reach against the sector max, and emits `HashMap<SystemId, HeatCell>` keyed by system id (colour from `faction_style_by_id`). The MAP panel forwards the map through `SectorView::heatmap` so the falloff renders without extra plumbing. |
| C8 influence-field overlay | INFLUENCE FIELD runs `sectorforge::influence_field::build` and samples the `dominant` cell at every system coord, scaling cell `score` into `HeatCell::intensity`. Both overlays clear when the toggle is set back to OFF; the MAP panel only pays the derivation when an overlay is on. |
