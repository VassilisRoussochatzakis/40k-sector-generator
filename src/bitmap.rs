...
fn render(sector: &GeneratedSector, scale: u32) -> RgbaImage {
    ...
    let map_w = (g.margin as f32 * 2.0
        + sector.width as f32 * horiz_step
        + horiz_step / 2.0) as i32;
...
fn draw_legend(img: &mut RgbaImage, sector: &GeneratedSector, map_w: i32, g: &Geom) {
    ...
    y += line_h + 4 * g.scale;
    draw_text(img, x0, y, &format!("SEED: {}", short(&sector.seed, 20)), TEXT_DIM, body);
    y += line_h - 4 * g.scale;
    draw_text(
        img,
        x0,
...
fn glyph(c: char) -> [u8; 7] {
    ...
    match up {
        ...
        _ => [0b00000, 0b11111, 0b10001, 0b10001, 0b10001, 0b11111, 0b00000],
    }
}
