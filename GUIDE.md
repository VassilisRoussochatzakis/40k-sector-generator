...
- **Worker panics surface as failures, not dead channels.** `spawn_job`
  ...
  returns `None` from `FromJobPanic` (e.g. `DerivationPayload`, since every
  derivation is total and a panic just leaves the kind stale to re-dispatch)
  and falls back to the `Disconnected` path. Under the release profile
  (`panic = "abort"`) `catch_unwind` is a no-op,
...
[`info_panel`](gui-core/src/info_panel/mod.rs) text helpers (`title` /
`section` / `body` / `dim` / `kv`, used in **both** apps) were likewise
flipped from `FontId::monospace` to proportional `.size(..)` at the same
theme-aligned sizes; the former `ui_kit::mono_title`/`mono_section`/`mono_body`/
`mono_dim` one-liner wrappers had no callers outside these and were later
inlined directly into `info_panel`. The single deliberate hold-out is the **segmentum map-canvas
...
pre-sized dropdown), and the `ui_kit::kv` key/value row helper for tabular
panels. Like `gui-core::nav` it takes
`&mut Ui` + plain data and has **no** `BuilderState` dependency, so both apps
share it. `info_panel` routes its `kv` row through it directly (its
`title`/`section`/`body`/`dim` primitives are local, sized off the same
`ui_kit`-re-exported type scale).
...
`ui_kit::reading_column(ui, |ui| …)` width-caps prose / markdown to a
readable line length (hardcoded to `720.0`). Both keep the same `&mut Ui` +
...
- *Design tokens* ([gui-core/src/design.rs](gui-core/src/design.rs), `§DESIGN`) —
  ...
  **spacing** grid (`SPACE_XS`…`SPACE_LG`), one **radius** family (`RADIUS_SM/MD`
...
- *Star-map flourishes* ([gui-core/src/sector_view/render.rs](gui-core/src/sector_view/render.rs),
...
[sector_view/render.rs](gui-core/src/sector_view/render.rs), then disappear below a ~3px
...
`SectorView` converts `SystemKind` into that token before painting glyphs,
then the renderer and `RenderMapTheme` match on the token only; region
condition colors are painted by matching `RegionConditionKind` directly (no
intermediate token). When a new system kind or region condition is added,
the compiler points at the one conversion / paint match that must be
updated, keeping the viewer, editor MAP panel, and builder MAP tab from
drifting.
...
[derivation_jobs.rs](builder/src/builder/derivation_jobs.rs) holds
`compute_chronicle`, the one derivation with real logic beyond a bare
`derive_with` call (it preserves `manual = true` events); the other six
overlays (relations / personae / hooks / sites / missions / prose) are
one-line `sectorforge::*::derive_with`/`derive_with_threshold` passthroughs
called directly from both the worker and the synchronous `recompute_*` in
[state/derivations.rs](builder/src/builder/state/derivations.rs). Either way
each takes owned/borrowed snapshots (sector + cloned cfg + scalars) with no
`&mut self`, so both call sites compute byte-identical output from identical
inputs. `dispatch_background_derivations`
...
* `coupling_flags_dead_world_with_population`, `coupling_flags_uninhabited_with_government`, `coupling_silent_on_normal_world`, `pinned_world_refuses_regen`, `enum_picker_variants_match_worlds_authoritative_set` in [builder/src/builder/panels/world/mod.rs](builder/src/builder/panels/world/mod.rs).
...
| Live preview | The shared `SectorView` widget fed from `session.preview` — the partial `GeneratedSector` produced by the off-thread prefix runner (`spawn_prefix` → `pump_prefix`, `IterativeJobResult`, stale revisions dropped). While a run is in flight the header shows a **real per-stage progress bar with an inline percentage**: `spawn_prefix`'s worker feeds `JobContext::set_progress` a monotonic fraction from `preview_progress_fraction(&SectorProgress, through)` (each engine `Stage` owns a `1/(through+1)` slice; the long per-item stages interpolate from their `current/total` counters) — not the old constant `0.5` that read as a frozen, half-full bar. The empty preview area distinguishes three states: *running* (live `ui.spinner()` + "Building preview…"), *Size step* (grid hint), and *idle/failed* ("No preview yet — press Next / Re-roll"). A missing worlds catalog now surfaces an explicit `feedback.last_command_error` instead of a silent no-op. (Fraction logic is UI-only — no effect on generated output; covered by `progress_bar_tests`.) |
...
`BuilderState` gains one field, `iterative_gen: Option<IterativeGenSession>`, holding the working `AppConfig`, root seed, `RerollNonces`, current step, cached `preview`, destination folder, the transient `regions_override`, the transient `catalogs_override` + `faction_palette` (see below), and the background job handle. The whole session is **transient** (the §V2 / command-bus carve-out): it is written directly, never through the bus, never serialised into `sector.json`, and never on the undo stack.

**Blank-builder launch (`catalogs_override`).** The wizard reads the world pool from `BuilderState::data_catalogs`, which is empty (`worlds == None`) until a project is opened. Launching ITERATIVE with no project open therefore used to dead-end at every step — the preview never ran, "re-roll this step" had no visible effect, and commit failed with *"No worlds catalog is loaded"*. The PROJECT-tab entry ([panels/project.rs](builder/src/builder/panels/project.rs)) now detects `data_catalogs.worlds.is_none()` and seeds the session's `catalogs_override: Option<DataCatalogs>` (+ matching `config.inputs`) from the bundled `_base` preset via `project_io::load_base_catalogs` — the same catalog set the one-shot `random` generator scaffolds. The override is pure transient session state like `regions_override`: `spawn_prefix` / `commit_new_project` swap it into `data_catalogs` *only* while assembling the `ProjectInput` (then restore), so a preview never persistently mutates the document; the commit boundary additionally installs it into the freshly written project so worlds.toml / factions.toml / … are written. At launch the source faction roster is **moved out** of the override into the session's `faction_palette` and the override's `factions` is **emptied**, so the Factions step opens blank and the user rebuilds the roster from the palette (see below). Because of that the override is now seeded in **both** launch paths: a blank builder seeds the whole catalog set from `_base`; a project-open launch seeds a clone of the live catalogs (the project's own roster becomes the palette, the source files untouched) instead of leaving the override `None`. Covered by `blank_builder_commit_seeded_from_base_writes_full_project`.
...
`Subsector`, `SubsectorConfig`, `SubsectorBuildError`,
...
| [gui-core/src/sector_view/](gui-core/src/sector_view/mod.rs) | Hex map render widget — split (F3) into view / render / cache |
