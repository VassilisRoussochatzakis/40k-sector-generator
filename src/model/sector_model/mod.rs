...
enum_slug!(SystemKind {
    Star => "star",
    SpecialLocation => "special_location",
    BlackHole => "black_hole",
    WarpAnomaly => "warp_anomaly",
...
impl GeneratedSector {
    ...
    /// Build an `id → systems[]` index for callers that do many `get_system`
    /// lookups in a loop, turning repeated O(n) scans into one O(n) build plus
    /// O(log n) lookups (A7). Deliberately **not** cached on the sector — the
    /// caller owns the map, so it can never go stale against a mutation and
    /// nothing is serialized into `sector.json`. Build it once, reuse it across
    /// the loop, drop it; for a one-off lookup use [`Self::get_system`].
    #[must_use]
    pub fn build_system_index(&self) -> BTreeMap<SystemId, usize> {
        self.systems
            .iter()
            .enumerate()
...
enum_slug!(ClaimType {
    LegalSovereignty => "legal_sovereignty",
    ImperialMandate => "imperial_mandate",
    TreatyRight => "treaty_right",
    ReligiousMandate => "religious_mandate",
    DynasticRight => "dynastic_right",
    CommercialCharter => "commercial_charter",
    MilitaryOccupation => "military_occupation",
    AncientDomain => "ancient_domain",
    HuntingGround => "hunting_ground",
    CovertWrit => "covert_writ",
    Rebellion => "rebellion",
});
