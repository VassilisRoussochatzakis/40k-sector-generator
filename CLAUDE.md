...
| [gui-core/src/sector_view.rs](gui-core/src/sector_view.rs) | Read-only hex map render. `SectorGeom::hit_route` (§CTX1 Phase 5) and `SectorMapCache::region_for_hex` (§CTX1 Phase 5) back the right-click route + region-hex schemas in `builder::panels::map::resolve_sector_context`. |
...
| [builder/src/builder/state/mod.rs](builder/src/builder/state/mod.rs) | `BuilderState` struct (§D5) + `new_blank` constructor + `default_config` + slice facade. Carries §CTX0 `scroll_target: Option<&'static str>` consumed by `panels/system.rs::show` to scroll the Star section into view when the in-system map's central star is clicked, and §CTX1 `sector_context_menu: Option<SectorContextMenu>` consumed by `panels/map.rs::show_sector_context_menu` to render the right-click menu on the sector map. Phase 3 adds `pending_bulk_rename: Option<PendingBulkRename>` driven from the multi-selection menu's `BULK RENAME…` row and rendered by `panels/map.rs::show_bulk_rename_dialog`. Phase 4 adds `partial_regen_anchor: Option<HexCoord>` armed by the MAP empty-hex menu's `START PARTIAL REGEN HERE` item and consumed by `panels/map.rs::apply_partial_regen_anchor_click` on the next primary click to populate `partial_regen_rect`. Phase 5 adds `pending_region_rename: Option<PendingRegionRename>` armed by the region-hex menu's `RENAME REGION…` row and rendered by `panels/map.rs::show_region_rename_dialog`, which dispatches `BuilderCommand::RenameRegion`. Phase 6 adds `system_context_menu: Option<SystemContextMenu>` armed by `panels/system_map.rs::arm_system_context_menu` on secondary-click inside the embedded `SystemView`, plus `pending_world_rename: Option<PendingWorldRename>` for the §6.7 `RENAME…` dialog. Phase 7 adds `last_menu_action: Option<String>` populated by `apply_sector_menu_action` / `apply_system_menu_action` via the new `*_menu_action_label` helpers and tailed in `panels/status.rs` as `ctx_menu: <schema> :: <item>`. In-memory only — dropped by `session::SessionFile` round-trip. |
| [builder/src/builder/state/types.rs](builder/src/builder/state/types.rs) | UI/dialog types: `BuilderTab`, `MapTool`, `ControlOverlay`, `ModalKind`, `HealthLevel`, `JobHandle`, `PartialRegenRect`, `Pending*` (incl. §CTX1 Phase 3 `PendingBulkRename`, Phase 5 `PendingRegionRename`, Phase 6 `PendingWorldRename`), `MapViewCache`, `HistoryWizardState`, `HistoryAnchorKind`, `TickLogEntry`, `TickLogScope`, §CTX1 `SectorContextMenu` (carries `bulk_delete_confirm` for the Phase 3 inline DELETE-ALL gate) + `SectorMenuTarget` (Phase 5 populates the `Route` and `RegionHex` variants), Phase 6 `SystemContextMenu` + `SystemMenuTarget` (Star / World / EmptyOrbit / Background), `DEFAULT_*` |
...
| [builder/src/builder/panels/map.rs](builder/src/builder/panels/map.rs) | §S1 + §R2 MAP tab — `show_hex_map` + `handle_click` + `handle_drag_drop`, ADD ROUTE preview, rect-select. §CTX1 sector right-click surface: `resolve_sector_context`, `apply_sector_menu_action`, `render_empty_hex_menu` / `render_system_menu` / `render_multi_selection_menu` / `render_route_menu` / `render_region_hex_menu` (Phase 2/3/5), `show_sector_context_menu` + `should_dismiss_sector_context_menu` (Phase 1), `show_bulk_rename_dialog` + `show_region_rename_dialog` transient dialogs (Phase 3/5), `apply_partial_regen_anchor_click` (Phase 4), `menu_anchor_pivot` viewport-clamp helper + `sector_menu_action_label` telemetry helper (Phase 7). |
| [builder/src/builder/panels/system.rs](builder/src/builder/panels/system.rs) | §S2 inspector + §CTX0 `show_system_map_section` that embeds `gui_core::system_view::SystemView` at the top of the SYSTEM tab and routes its `secondary_clicked` response through `panels/system_map::arm_system_context_menu` for the §CTX2 in-system right-click menu. Hosts the `apply_bulk_*` `pub(crate)` helpers reused by the multi-selection right-click menu in `panels/map.rs`. |
...
| [builder/src/builder/panels/system_map.rs](builder/src/builder/panels/system_map.rs) | §CTX1 Phase 6 in-system right-click menu (`§6.6`..`§6.9`). Owns `SystemMenuAction` + `resolve_system_context` + `apply_system_menu_action` + `arm_system_context_menu` + `show_system_context_menu` + `show_world_rename_dialog`. Star schema dispatches `BuilderCommand::SetStar` / `SetStarSpectral`; world schema dispatches `RenameWorld` / `SetWorldOrbit` / `RemoveWorld` (and uses `AddWorld` + an inline patch for `DUPLICATE WORLD`); background schema dispatches `AddWorld` for the empty case and calls `super::orbital::derive_and_apply_orbital_assets` for `DERIVE ORBITAL ASSETS`. Phase 7 polish adds `menu_selection_override` (hover-ring re-use via `SystemSelection`) and `system_menu_action_label` (status-bar telemetry tail) + reuses `super::map::menu_anchor_pivot` to flip the menu's anchor pivot when the cursor sits on the right/bottom half of the viewport. |
