...
| TF-NT-2 | `ControlScore`/`DisplayImportance`/`ProjectedPower` newtypes in `src/analysis/scores.rs`. | **Deeper re-assessment (kept deferred):** `total_projection`/`display_importance` are a method + free fn, not stored fields, so adoption ripples to ~15+ arithmetic/comparison sites across the *serialized* analysis layer (importance/prose/power_projection/analytics/render/diff/world_ecs), each needing `.get()`. Pure type-decoration, golden-adjacent, zero behaviour change — do it only when a consumer compares scores across the three types (none does today). |
