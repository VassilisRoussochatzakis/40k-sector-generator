...
pub(crate) struct Plan {
    ...
    pub route_ids: Vec<sectorforge::ids::RouteId>,
...
pub struct HazardNote {
    pub route_id: sectorforge::ids::RouteId,
    pub from: sectorforge::ids::SystemId,
    pub to: sectorforge::ids::SystemId,
...
fn reconstruct(
    ...
    while let Some((p, route_id)) = prev.get(cur) {
        hops_rev.push(sectorforge::ids::SystemId::new(*p));
        routes_rev.push(sectorforge::ids::RouteId::new(*route_id));
        cur = p;
    }
    hops_rev.reverse();
    routes_rev.reverse();
...
fn economy_lifelines(sector: &GeneratedSector) -> HashMap<sectorforge::ids::RouteId, String> {
    let mut out: HashMap<sectorforge::ids::RouteId, String> = HashMap::new();
    if !sector.economy.enabled {
        return out;
    }
    let sys_econ: HashMap<&str, &sectorforge::economy::SystemEconomy> = sector
        .economy
        .systems
        .iter()
    ...
    for crit in critical {
        for sys in &sector.systems {
            ...
            let candidates: Vec<&sectorforge::sector_model::GeneratedRoute> = sector
                .routes
                .iter()
                .filter(|r| {
