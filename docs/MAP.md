...
| [gui-core/src/sector_view.rs](../gui-core/src/sector_view.rs) | Read-only hex map render. `SectorGeom::hit_route` and `SectorMapCache::region_for_hex` back the right-click route + region-hex schemas in `builder::panels::map::context_menu::resolve_sector_context`. **§BEAUTY** live-only void flourishes (`paint_star_dust` / `paint_vignette` / `paint_chart_frame`, gated by `is_dark` + canvas size) paint on the egui path only — the golden PNG/SVG exporters are untouched; the gui-core `map_snapshots` goldens cover them. |
