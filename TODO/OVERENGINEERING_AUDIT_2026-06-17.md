...
- [ ] `shrink` · **−10** `tests/it/random_sector_tests.rs:324` — 3rd hand-rolled recursive `walkdir`; flat `read_dir` if presets aren't nested. **NOT APPLIED: presets dir IS nested, so the flat-`read_dir` swap doesn't apply — recursive walk kept.**
- [x] `shrink` · **−8** `tests/it/analytics_and_presets.rs:7` — `presets_dir`/`goldens_dir` each defined twice → shared.rs.
- [ ] `shrink` · **−8** `src/model/sector_model/mod.rs:660` — `RouteType::key()` duplicates `as_slug()` slugs (low: parity-test-guarded, many callers). **REJECTED (determinism): `key()` feeds the FNV-1a route-pattern hash at `routes_view.rs:132` → PNG/SVG/HTML golden bytes, and NO test guards `key()==as_slug()` (the parity test only covers `as_slug()`↔serde). Merging would couple byte-stable output to the serde slug. Kept deliberately separate.**
