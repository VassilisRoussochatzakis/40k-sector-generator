...
pub fn extract_example(name: &str) -> Result<(TempDir, Utf8PathBuf), String> {
    ...
    let example_dir = EXAMPLES_DIR.get_dir(name)
...
fn extract_dir_recursive(dir: &Dir, target: &Utf8PathBuf) -> Result<(), String> {
    fs::create_dir_all(target.as_std_path()).map_err(|e| format!("failed to create dir {target}: {e}"))?;
    
    for entry in dir.files() {
        let file_path = target.join(entry.path().file_name().unwrap().to_str().unwrap());
        fs::write(file_path.as_std_path(), entry.contents()).map_err(|e| format!("failed to write file {file_path}: {e}"))?;
    }
